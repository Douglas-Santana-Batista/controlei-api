...
import {
  ...
  subcategoryupdateParamsSchema,
  subcategorydeleteParamsSchema,
  updateSubCategorybodySchema,
  subcategoryGetSchema,
  subcategorygetuser,
} from "../schemas/subCategorySchema";
import prisma from "../models/prisma";
import { AppError } from "../utils/AppError";

export const createsubCategory: RequestHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    const { description, value, payment_type, date, financial_flow } =
      subCategorybodySchema.parse(req.body);
    const { id_user, id_category } = subcategoryIdParamsSchema.parse(
      req.params
    );
...
export const getAllsubCategory: RequestHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const getSubcategory: RequestHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    const { id_category, id_user, id_subcategory } = subcategoryGetSchema.parse(
      req.params
    );
...
export const updatesubCategory: RequestHandler = async (
  ...
  try {
    const { id_user, id_category, id_subcategory } =
      subcategoryupdateParamsSchema.parse(req.params);
    ...
    const dataToUpdate = Object.fromEntries(
      Object.entries(bodyData).filter(([, v]) => v !== undefined)
    );
    ...
    res
      .status(200)
      .json({ message: "Subcategory updated successfully", updatesubCategory });
  } catch (error) {
    next(error);
  }
};

export const deletesubCategory: RequestHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  try {
    const { id_subcategory, id_category, id_user } =
      subcategorydeleteParamsSchema.parse(req.params);
