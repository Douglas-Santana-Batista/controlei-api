...
import { subcategoryIdParamsSchema, subCategorybodySchema, subcategoryupdateParamsSchema, subcategorydeleteParamsSchema, updateSubCategorybodySchema } from '../schemas/subCategorySchema'
...
export const updatesubCategory: RequestHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id_user, id_subcategory } = subcategoryupdateParamsSchema.parse(req.params)
...
export const deletesubCategory: RequestHandler = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { id_subcategory} = subcategorydeleteParamsSchema.parse(req.params)

        const deletesubCategory = await prisma.subcategory.delete({
            where: { id_subcategory }
        })
        res.status(200).json({ message: "Subcategory deleted successfully ", deletesubCategory })
    } catch (error) {
        next(error)
    }
}