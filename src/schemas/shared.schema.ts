...
export const MonetaryValueSchema = z.union([z.string(), z.number()]).transform((value) => {
  let strValue = typeof value === "number" ? value.toLocaleString("pt-BR", { minimumFractionDigits: 2 }) : value.toString();
  ...
  return new Prisma.Decimal(`${integerPart.replace(/\D/g, "")}.` + `${decimalPart.padEnd(2, "0").slice(0, 2)}`);
});
...
export const idSchema = z
  .string()
  .refine((val) => !isNaN(parseInt(val, 10)), { message: "ID must be a valid number" })
...
export const toUpperCase = (value: unknown) => {
