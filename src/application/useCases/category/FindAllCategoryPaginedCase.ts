...
export class FindAllPaginatedCategoryCase {
  ...
  async execute(params: PaginationParams): Promise<PaginationResult<Category>> {
    try {
      if (params.page < 1) throw new AppError("Page must be at least 1", 400);
      if (params.limit < 1 || params.limit > 100) throw new AppError("Limit must be between 1 and 100", 400);
